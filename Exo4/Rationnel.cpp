...
{
    if (myDenom == 0) throw CException("Error in RATIONNEL::Rationnel : denom must be different from 0", KExcDivZero);
    simplify ();
...
RATIONNEL RATIONNEL::operator / (const Rationnel & r)
    const
{
    if(r.myNum == 0) throw CException("error in RATIONNEL::operator/ : can't divide by zero", KExcDivZero);
